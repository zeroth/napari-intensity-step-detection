...
class NLayerWidget(QWidget):
    def __init__(self, napari_viewer : napari.viewer.Viewer=None, parent:QWidget=None):
        ...
        self.layer_filter = {"Image": napari.layers.Image, "Label": napari.layers.Labels}
        ...
        if self.viewer:
            self.viewer.layers.events.inserted.connect(self.viewer_layer_updated)
            self.viewer.layers.events.removed.connect(self.viewer_layer_updated)
