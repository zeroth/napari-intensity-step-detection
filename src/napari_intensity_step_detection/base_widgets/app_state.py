...
from napari_intensity_step_detection.tracks_writer import track_stats_state_write
from napari_intensity_step_detection.tracks_reader import track_stats_state_reader
import warnings
...
class AppState(QObject):
    ...
    def save(self, file_path):
        _saved = track_stats_state_write(file_path, self)
        if _saved == file_path:
            napari.utils.notifications.show_info(f"Saved to {_saved}")

    def open(self, file_path):
        data = track_stats_state_reader(file_path)
        if data:
            # self._data = data
